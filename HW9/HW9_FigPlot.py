...
annoate_dict = {"HLLC": ["navy", "D", 2]}

fig, ax = plt.subplots(4, 3, figsize=(12, 12))

res_Table = {}

for root, dirs, files in os.walk("HW9_res"):
            ...
            if len(re.findall(r".txt", f))>0:
                ...
                if schemeName == "Exact":
                    ...
                    ax[int(caseNo)][var_dict[var]].scatter(x_loc, resArray, s=2, c=annoate_dict[schemeName][0],
                                                        marker=annoate_dict[schemeName][1], label="5th WENO", zorder=annoate_dict[schemeName][2])
...
#plt.show()
plt.savefig("HW9_WENO.png", dpi=400)

# Calculate errors
print("Para", "\t", "rho-10error", "\t", "rho-90error", "\t", "U-10Error", "\t", "U-90error", "\t", "p-10Error", "\t", "p-90Error")
for nx in [100, 200, 500, 1000]:
    rhoError = np.abs(res_Table["HLLC" + str(nx) + "Rho"] - res_Table["Exact"+str(nx)+"Rho"])
    uError = np.abs(res_Table["HLLC" + str(nx) + "U"] - res_Table["Exact" + str(nx) + "U"])
    pError = np.abs(res_Table["HLLC" + str(nx) + "P"] - res_Table["Exact" + str(nx) + "P"])
    rho90 = np.percentile(rhoError, 90)
